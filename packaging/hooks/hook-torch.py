...
if is_module_satisfies("PyInstaller >= 6.0"):
    ...
    # bioCN uses torch for CPU-only inference; skip collecting CUDA/nvidia libs.
    ...
    # libraries (because code in one of the libraries looks up the library file's location, but does not fully
    # resolve it), and prevent it from finding dynamically-loaded libraries in `torch/lib` directory. These symlinks
    # are not necessary on linux in general, and there should be no harm in suppressing them for all versions.
    #
