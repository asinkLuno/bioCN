...
GPU 加速的关键优化：
- **批量分析**：收集所有段落，在单次 GPU 调用中处理所有句子
- **设备自动选择**：自动检测并使用 GPU（如可用），否则回退到 CPU
- **性能提升**：GPU 环境下可获得 2-10x 加速（相比逐段处理）
...
3. **处理时间**：GPU 环境下处理速度快（推荐），CPU 环境下大型 EPUB 文件可能需要较长时间
4. **兼容性**：仅支持标准 EPUB 格式
5. **语言支持**：专门针对中文文本设计
...
- **GPU 批处理优化**：将所有段落在单次 GPU 调用中批量分析，GPU 上可获得 **2-10x 加速**
- 自动检测 GPU/CPU 环境并选择最优设备

**性能建议**：
- 大文件：使用 `--no-inline-css` + GPU 环境
- GPU 环境下处理速度显著快于 CPU（推荐使用 GPU 版本安装）
